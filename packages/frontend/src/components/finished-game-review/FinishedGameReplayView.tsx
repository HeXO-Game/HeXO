...
import ClockCard from '../game-screen/ClockCard';
import { getReplayClocks } from '../../utils/replayClocks';
import { getPlayerColor, getPlayerLabel } from '../../utils/gameBoard';
...
function FinishedGameReplayView({
    ...
    const clocks = useMemo(() => getReplayClocks(game, visibleMoveCount), [game, visibleMoveCount]);
                        ...
                        {({ resetView }) => (
                            <>
                                {game.gameOptions.timeControl.mode !== "unlimited" && (
                                    <div className="pointer-events-none absolute inset-x-0 top-0 grid grid-cols-2 gap-3 p-3 sm:p-5 max-w-xl mx-auto">
                                        {game.players.map(player => (
                                            <ClockCard
                                                key={player.playerId}
                                                label={player.displayName}
                                                timeMs={clocks[player.playerId]}
                                                markerColor={getPlayerColor(game.playerTiles, player.playerId, theme)}
                                                isHighlighted={activeMove?.playerId === player.playerId}
                                            />
                                        ))}
                                    </div>
                                )}
